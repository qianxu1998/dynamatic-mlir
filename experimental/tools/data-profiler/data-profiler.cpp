...
static cl::opt<ProfilerMode>
    mode("mode", cl::desc("Select profiler mode"),
         cl::values(clEnumValN(ProfilerMode::Data, "data",
                               "Run data profiling only"),
                    clEnumValN(ProfilerMode::Frequency, "frequency",
                               "Run frequency profiling only"),
                    clEnumValN(ProfilerMode::Both, "both", "Run both")),
         cl::init(ProfilerMode::Both), cl::cat(mainCategory));
...
int main(int argc, char **argv) {
  ...
  if (mode != ProfilerMode::Data || traceLoggerPtr || bbListLoggerPtr) {
    if (failed(
            simulate(funcOp, args, profiler, traceLoggerPtr, bbListLoggerPtr)))
      return 1;
  }
  ...
  if (mode == ProfilerMode::Frequency || mode == ProfilerMode::Both)
    profiler.writeStats(/*dotFormat=*/false, outputFrequencies);
