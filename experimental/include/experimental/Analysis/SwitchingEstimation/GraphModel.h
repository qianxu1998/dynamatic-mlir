...
public:
  ...
  // This function will calculate the total valid and ready switching for the node
  ...
  std::map<std::string, std::vector<int>> dataOut;  // Map from succeeding node name to the corresponding output channel dict
...
class AdjGraph {
public:
  AdjGraph(CFDFC* cfdfc, const TimingDatabase& timingDB, 
  ...
  AdjGraph(const TimingDatabase& timingDB, const unsigned &II, 
            ...
            std::vector<std::pair<std::string, std::string>> &allBackedges, const double &targetPeriod);
  ...
  std::shared_ptr<AdjNode> createNodeFromOperation(Operation *op,
  ...
  // This function finds the cond_src node for all cond_br nodes and data/address src for store nodes in the dfg
