...
// we update 1) database nodes  2) controlmerge nodes 3) influenced multiplexer
// nodes
...
// This function find all glitching nodes within different MGs, "S", "E", and
// "T" segments will be ignored
...
// This function calculates a single glitching value based on the given input
// operands and node type
...
// This function gets the cond_value of for the cond_br node for data channel
// update
...
void printMuxToSrcNodeMap(
    const llvm::StringMap<std::map<std::string, std::string>> &muxToSrcNodeMap);
