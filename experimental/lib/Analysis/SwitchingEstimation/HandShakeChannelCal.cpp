...
void updateMGBufferSwitching(SwitchingInfo &switchInfo, std::string selMG,
  ...
  IISet selMGUniverseSet(selMGII, true);
  ...
  for (const auto &selBuffName : selAdjGraph->orderedNodeName) {
    if (contains(selBuffName, "buffer")) {
      ...
      selBuffNode->totalHandshakeSwitchingCounting();
    }
  }
}
...
void mgHandshakeSwitchingCounting(SwitchingInfo &switchInfo, std::string selMG,
  ...
  while (pendingList.size() > 0) {
    ...
    for (auto &selNode : pendingList) {
      //
      nodeHandshakeUpdate(switchInfo, selNode, selMG, selMGII, true);
  ...
  // TODO: Add logic to update the unfinished list, if any
  // for (const auto &entry : selAdjGraph->nodes) {
  //   entry.second->totalHandshakeSwitchingCounting();
  // }
...
void nodeHandshakeUpdate(SwitchingInfo &switchInfo, std::string &selNode,
  ...
  for (const auto &selPre : selNodeStoringDict[selNode]->pres) {
    ...
    if (selNodeStoringDict[selPre]->validSignal.find(selNode) !=
      ...
      // TODO: Optimize the storing and updating mechanism
  ...
  for (const auto &selSuc : selNodeStoringDict[selNode]->sucs) {
    ...
    if (selNodeStoringDict[selSuc]->readySignal.find(selNode) !=
      ...
      // The nReady signal is not there yet
...
breakHandshakeUpdateDeadlock(SwitchingInfo &switchInfo,
  ...
  for (auto &selNode : selNodeList) {
    ...
    // TODO: Need to further verify the following criteria
