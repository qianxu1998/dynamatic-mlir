...
def main(output_dir, kernel_name, hdl, clock_period, stage, flatten_hierarchy,
    ...
    synth_lines += [
        ...
        "                # Extract the floating point power value",
        "                if {[regexp -nocase {power=\"([0-9\\.eE+-]+)\"} $line -> pwr_w]} {",
        "                    # Double-check it's a valid number",
        "                    if {[string is double -strict $pwr_w]} {",
        "                        # Accumulate the power (in Watts for now)",
        "                        if {[info exists mod_power($mod_name)]} {",
        "                            set mod_power($mod_name) [expr {$mod_power($mod_name) + $pwr_w}]",
        "                        } else {",
        "                            set mod_power($mod_name) $pwr_w",
        "                        }",
        "                    }",
        "                }",
        "            }",
