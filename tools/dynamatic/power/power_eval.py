...
def main(output_dir, kernel_name, hdl, clock_period, stage, flatten_hierarchy,
    ...
    # Collect RTL sources.
    # Verilog kernels may still contain LSQ blocks generated as VHDL, so include
    # both languages for the Vivado mixed-language flow.
    rtl_extensions = [".v", ".vhd"] if hdl == "verilog" else [".vhd"]
    ...
    synth_lines += [
        ...
        "",
        "# =============================================================",
        "# Helper Procedure: Generate High-Precision uW Power Report",
        "# =============================================================",
        "proc generate_detailed_power {saif_file rpt_file xpe_file stage_name detailed_rpt_file} {",
        "    set detail_fp [open $detailed_rpt_file w]",
        "",
        "    set sep_line \"==================================================================\"",
        "    set dash_line \"------------------------------------------------------------------\"",
        "",
        "    puts \"\\n$sep_line\"",
        "    puts \" Power Analysis Stage: $stage_name\"",
        "    puts \"$sep_line\"",
        "    puts $detail_fp $sep_line",
        "    puts $detail_fp \" Power Analysis Stage: $stage_name\"",
        "    puts $detail_fp $sep_line",
        "",
        "    # 1. Read the SAIF file",
        "    if {[file exists $saif_file]} {",
        "        read_saif -file $saif_file",
        "    } else {",
        "        set err_line \"Error: SAIF file not found: $saif_file\"",
        "        puts $err_line",
        "        puts $detail_fp $err_line",
        "        puts $detail_fp $sep_line",
        "        close $detail_fp",
        "        return",
        "    }",
        "",
        "    # 2. Generate standard text report AND the high-precision XPE (XML) file",
        "    report_power -file $rpt_file -hierarchical_depth 0 -verbose -xpe $xpe_file -name pwr_1",
        "",
        "    # 3. Parse the XPE XML file to extract and sum unrounded power metrics",
        "    set detail_header \"Detailed Module Power Report (uW) - Extracted from XPE\"",
        "    puts \"\\n$detail_header\"",
        "    puts \"$dash_line\"",
        "    puts $detail_fp \"\"",
        "    puts $detail_fp $detail_header",
        "    puts $detail_fp $dash_line",
        "",
        "    if {[file exists $xpe_file]} {",
        "        set fp [open $xpe_file r]",
        "",
        "        # Create an empty associative array to hold the sum for each module",
        "        array set mod_power {}",
        "        set total_hier_power_w 0.0",
        "",
        "        # Read the XML file line-by-line",
        "        while {[gets $fp line] >= 0} {",
        "            # Extract the hierarchical module name (hierName)",
        "            if {[regexp -nocase {hierName=\"([^\"]+)\"} $line -> mod_name]} {",
        "                set pwr_w 0.0",
        "                set sp_w  0.0",
        "",
        "                # Extract internal logic power (power=)",
        "                if {[regexp -nocase {power=\"([0-9\\.eE+-]+)\"} $line -> val_pwr]} {",
        "                    if {[string is double -strict $val_pwr]} { set pwr_w $val_pwr }",
        "                }",
        "",
        "                # Extract internal signal/routing power (sp=)",
        "                if {[regexp -nocase {sp=\"([0-9\\.eE+-]+)\"} $line -> val_sp]} {",
        "                    if {[string is double -strict $val_sp]} { set sp_w $val_sp }",
        "                }",
        "",
        "                # Combine Logic + Routing power",
        "                set combined_w [expr {$pwr_w + $sp_w}]",
        "",
        "                if {$combined_w > 0} {",
        "                    if {[info exists mod_power($mod_name)]} {",
        "                        set mod_power($mod_name) [expr {$mod_power($mod_name) + $combined_w}]",
        "                    } else {",
        "                        set mod_power($mod_name) $combined_w",
        "                    }",
        "                }",
        "            }",
        "        }",
        "        close $fp",
        "",
        "        # Convert to uW, sort alphabetically, and print",
        "        foreach mod [lsort [array names mod_power]] {",
        "            set pwr_uw [expr {$mod_power($mod) * 1000000.0}]",
        "            set total_hier_power_w [expr {$total_hier_power_w + $mod_power($mod)}]",
        "",
        "            # Only print modules consuming > 0 uW",
        "            if {$pwr_uw > 0} {",
        "                set mod_line [format \"%-50s : %10.3f uW\" $mod $pwr_uw]",
        "                puts $mod_line",
        "                puts $detail_fp $mod_line",
        "            }",
        "        }",
        "",
        "        # Print the total hierarchical sum",
        "        set total_hier_power_uw [expr {$total_hier_power_w * 1000000.0}]",
        "        set total_line [format \"%-50s : %10.3f uW\" \"TOTAL HIERARCHICAL POWER SUM\" $total_hier_power_uw]",
        "        puts \"$dash_line\"",
        "        puts $total_line",
        "        puts $detail_fp $dash_line",
        "        puts $detail_fp $total_line",
        "",
        "    } else {",
        "        set err_line \"Error: XPE file was not generated at $xpe_file.\"",
        "        puts $err_line",
        "        puts $detail_fp $err_line",
        "    }",
        "    puts \"$sep_line\\n\"",
        "    puts $detail_fp $sep_line",
        "    puts $detail_fp \"Detailed report written to: $detailed_rpt_file\"",
        "    close $detail_fp",
        "",
        "    # 4. Reset switching activity for the next run",
        ...
        "# =============================================================",
        "",
        "# Set up some variables for cleaner paths",
        f"set SAIF_DIR \"{verify_dir}\"",
        f"set PWR_DIR  \"{power_dir}\"",
        "",
        "# --- Run Analysis for Post-Synthesis Functional ---",
        "generate_detailed_power \\",
        "    \"$SAIF_DIR/post_synth_fun.saif\" \\",
        "    \"$PWR_DIR/post_synth_fun_power.rpt\" \\",
        "    \"$PWR_DIR/post_synth_fun_power.xpe\" \\",
        "    \"Post-Synthesis Functional\" \\",
        "    \"$PWR_DIR/post_synth_fun_detailed_power_report.rpt\"",
        "",
        "# --- Run Analysis for Post-Synthesis Timing ---",
        "generate_detailed_power \\",
        "    \"$SAIF_DIR/post_synth_time.saif\" \\",
        "    \"$PWR_DIR/post_synth_time_power.rpt\" \\",
        "    \"$PWR_DIR/post_synth_time_power.xpe\" \\",
        "    \"Post-Synthesis Timing\" \\",
        "    \"$PWR_DIR/post_synth_time_detailed_power_report.rpt\"",
    ]
    synth_block = "\n".join(synth_lines) + "\n"
    ...
    if stage == DesignStage.IMPL:
        impl_lines = [
            ...
            "# =============================================================",
            "",
            "# --- Run Analysis for Post-Implementation Functional ---",
            "generate_detailed_power \\",
            "    \"$SAIF_DIR/post_impl_fun.saif\" \\",
            "    \"$PWR_DIR/post_impl_fun_power.rpt\" \\",
            "    \"$PWR_DIR/post_impl_fun_power.xpe\" \\",
            "    \"Post-Implementation Functional\" \\",
            "    \"$PWR_DIR/post_impl_fun_detailed_power_report.rpt\"",
            "",
            "# --- Run Analysis for Post-Implementation Timing ---",
            "generate_detailed_power \\",
            "    \"$SAIF_DIR/post_impl_time.saif\" \\",
            "    \"$PWR_DIR/post_impl_time_power.rpt\" \\",
            "    \"$PWR_DIR/post_impl_time_power.xpe\" \\",
            "    \"Post-Implementation Timing\" \\",
            "    \"$PWR_DIR/post_impl_time_detailed_power_report.rpt\"",
        ]
        impl_block = "\n".join(impl_lines) + "\n"
...
if __name__ == "__main__":
    ...
    p.add_argument("--vivado_cmd", type=str, required=False, help="Vivado command", default="vivado-2019.1.1-bt vivado")
