...
class Scope(MutableMapping):
    ...
    def __getitem__(self, key):
        if isinstance(key, RE_TYPE):
            pattern = '^' + re.escape(self.scope_name) + r'\.' + key.pattern
...
class VcdParser(object):
    ...
    ################################### Toggling Counting ###########################################
    @staticmethod
    def _is_unknown_state(value):
        # Keep compatibility with previous behavior: only previous unknown
        # states are ignored when counting transitions.
        return any(ch in ('x', 'X', 'z', 'Z') for ch in str(value))

    @staticmethod
    def _time_in_window(time, start_time=None, end_time=None,
                        start_exclusive=False, end_inclusive=True):
    ...
    def value_list_change_count(self, value_list, start_time=None, end_time=None,
                                start_exclusive=False, end_inclusive=True):
        """
        ...
        for i in range(1, len(value_list)):
            ...
            if (value_list[i][1] != value_list[i - 1][1]):
                if not self._is_unknown_state(value_list[i - 1][1]):
                    num_transitions += 1

        return num_transitions

    def _node_leaf_prefix_regex(self, node_name):
        """
            Build a regex that matches signal names whose leaf starts with
            '<node_name>_' (or exactly '<node_name>').
            This avoids false matches such as merge0 matching control_merge0_*.
        """
        return r'(?:^|\.)' + re.escape(node_name) + r'(?=_|$)'
    
    def get_node_handshake_toggle_count(self, node_name, start_time=None,
                                        end_time=None, start_exclusive=False,
                                        end_inclusive=True):
        """"
        ...
        node_prefix = self._node_leaf_prefix_regex(node_name)
        valid_signal_str = node_prefix + r'[^.]*_valid$'
        ready_signal_str = node_prefix + r'[^.]*_ready$'
        ...
        for valid_signal in matched_valid_signal_list:
            ...
            tmp_toggle_number = self.value_list_change_count(
                tmp_value_list,
                start_time=start_time,
                end_time=end_time,
                start_exclusive=start_exclusive,
    ...
    def get_node_dataout_toggle_count(self, node_name, start_time=None,
                                      end_time=None, start_exclusive=False,
                                      end_inclusive=True):
        """
        ...
        # Include common integer datapath output aliases used by generated RTL.
        # Branch-like nodes emit trueOut/falseOut rather than dataOut.
        node_prefix = self._node_leaf_prefix_regex(node_name)
        dataout_signal_str = (
            node_prefix
            + r'_(?:result|outs(?:_[0-9]+)?|dataOut(?:Array)?(?:_[0-9]+)?|'
    ...
    def get_node_toggle_count(self, node_name, start_time=None, end_time=None,
                              start_exclusive=False, end_inclusive=True):
        """
        ...
        self.get_node_handshake_toggle_count(
            node_name,
            start_time=start_time,
            end_time=end_time,
            start_exclusive=start_exclusive,
        ...
        self.get_node_dataout_toggle_count(
            node_name,
            start_time=start_time,
            end_time=end_time,
            start_exclusive=start_exclusive,
